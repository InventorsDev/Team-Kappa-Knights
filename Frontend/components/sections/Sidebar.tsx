...
const Sidebar = () => {
  ...
  return (
    ...
    <aside className="w-[8%] fixed h-[90vh] hidden md:flex flex-col justify-between bg-gray-10 border-r border-gray-300">
      ...
      <div className="flex flex-col gap-8 mt-3">
        {sidebarItems.map((item, idx) => (
          ...
          >
            <div className={`text-[16px] w-full flex flex-col items-center py-2   ${ pathname === item.link ? 'text-[#00BFA5] bg-[#EBFFFC] border-r-3 border-[#00BFA5]' : 'text-[#212121] bg-white' }`}>
            <Image src={pathname === item.link ? item.green : item.logo} width={32} height={32} alt={item.altText} />
            <p>{item.text}</p>
            </div>
          </Link>
        ))}
